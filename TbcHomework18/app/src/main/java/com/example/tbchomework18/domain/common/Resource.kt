package com.example.tbchomework18.domain.common
...
fun <DTO,DOMAIN>Flow<Resource<DTO>>.mapResource(mapper:(DTO)->DOMAIN):Flow<Resource<DOMAIN>>{
    return map { resource ->
        when(resource){
            is Resource.Success -> { Resource.Success(dataSuccess = mapper(resource.dataSuccess))}
            is Resource.Error -> { Resource.Error(errorMessage = resource.errorMessage)}
            is Resource.Loading -> { Resource.Loading(load = resource.loading)}
        }
    }
}
